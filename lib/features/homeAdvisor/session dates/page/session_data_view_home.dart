...
class _SessionDetailsViewHomeState extends State<SessionDetailsViewHome> {
                            ...
                            child: BorderRoundedButton(
                                ...
                                onPressed: () {
                                  ...
                                  if(advisorComment.text==''){
                                    ...
                                    updateSessionCubit.updateSession(data)
                                        .then((response) {
                                        ...
                                        children: [
                                          IconButton(
                                            ...
                                            onPressed: () async {
                                              ...
                                              // Add the second page (for pointers)
                                              pdf.addPage(
                                                pw.Page(
                                                  build: (pw.Context context) {
                                                    return pw.Column(
                                                            ...
                                                            pw.TableRow(
                                                              children: [
                                                                pw.Column(
                                                                  ...
                                                                  children: pointers3Temp.map((pointer) {
                                                                    return pw.Padding(
                                                                ...
                                                                pw.Column(
                                                                  crossAxisAlignment: pw.CrossAxisAlignment.end,
                                                                  mainAxisAlignment: pw.MainAxisAlignment.start,
                                                                  children: pointers2Temp.map((pointer) {
                                                                    return pw.Padding(
                                                                      padding: const pw.EdgeInsets.only(right: 3),
                                                                      child: pw.Text(
                                                                        pointer["text"] ?? '',
                                                                ...
                                                                pw.Column(
                                                                  crossAxisAlignment: pw.CrossAxisAlignment.end,
                                                                  mainAxisAlignment: pw.MainAxisAlignment.start,
                                                                  children: pointers1Temp.map((pointer) {
                                                                    return pw.Padding(
                                                                      padding: const pw.EdgeInsets.only(right: 3),
                                                                      child: pw.Text(
                                                                        pointer["text"] ?? '',
                                              ...
                                              // Add pages for advices (dynamic pagination)
                                              List<List<dynamic>> splitAdvicesIntoChunks(List<dynamic> advices, int chunkSize) {
                                                List<List<dynamic>> chunks = [];
                                                for (var i = 0; i < advices.length; i += chunkSize) {
                                                  chunks.add(advices.sublist(i, i + chunkSize > advices.length ? advices.length : i + chunkSize));
                                                }
                                                return chunks;
                                              }

                                              List<List<dynamic>> adviceChunks = splitAdvicesIntoChunks(advices, 10); // 10 rows per page

                                              for (var chunk in adviceChunks) {
                                                pdf.addPage(
                                                  pw.Page(
                                                    build: (pw.Context context) {
                                                      return pw.Column(
  ...
  Future<void> addPointers(int pointerId, int sessionId) async {
    ...
    try {
      final response = await dio.post(
          '${Constants.baseUrl}/api/advicor/session/add-session-pointer',
          options: Options(headers: {
            "api-password": Constants.apiPassword,
            "token": CacheHelper.getData(key: "token")
          }),
  ...
  Future<void> deletePointers(int sessionId, int pointerId) async {
    ...
    try {
      final response = await dio.post(
          '${Constants.baseUrl}/api/advicor/session/delete-session-pointer',
          options: Options(headers: {
            "api-password": Constants.apiPassword,
            "token": CacheHelper.getData(key: "token")
          }),
  ...
  Future<void> addAdvices(int adviceId, int sessionId) async {
    ...
    try {
      final response = await dio.post(
        '${Constants.baseUrl}/api/advicor/session/add-session-advice',
        options: Options(headers: {
          "api-password": Constants.apiPassword,
          "token": CacheHelper.getData(key: "token"),
        }),
  ...
  Future<void> deleteAdvices(int sessionId, int adviceId) async {
    ...
    try {
      final response = await dio.post(
          '${Constants.baseUrl}/api/advicor/session/delete-session-advice',
          options: Options(headers: {
            "api-password": Constants.apiPassword,
            "token": CacheHelper.getData(key: "token")
          }),
